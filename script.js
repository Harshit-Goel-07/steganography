...
document.getElementById("imageUploadEncrypt").addEventListener("change", function (event) {
  const file = event.target.files[0];
  if (file) {
    const reader = new FileReader();
    reader.onload = function (e) {
      document.getElementById("imagePreviewEncrypt").src = e.target.result;
    };
    reader.readAsDataURL(file);
  }
});

document.getElementById("imageUploadDecrypt").addEventListener("change", function (event) {
  const file = event.target.files[0];
  if (file) {
    const reader = new FileReader();
    reader.onload = function (e) {
      document.getElementById("imagePreviewDecrypt").src = e.target.result;
    };
    reader.readAsDataURL(file);
  }
});
...
function encodeText() {
  ...
  const img = document.getElementById("imagePreviewEncrypt"); // Correct ID
  ...
  image.onload = function () {
    ...
    const filename = prompt("Enter filename to save the encoded image (include .png, .jpg, or .jpeg extension):", "encoded_image.png");
    ...
    if (filename && (filename.endsWith(".png") || filename.endsWith(".jpg") || filename.endsWith(".jpeg"))) {
        ...
        alert("Invalid filename! Please include .png, .jpg, or .jpeg extension.");
    }
...
function decodeText() {
  const img = document.getElementById("imagePreviewDecrypt"); // Correct ID
